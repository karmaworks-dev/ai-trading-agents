'''
🕉️ Karma Dev's Research Agent 🕉️
...
You are Karma Dev's Trading Strategy Idea Generator 🕉️
...
def animate_loading(duration=3, message="Generating idea", emoji="🕉️"):
...
def animate_karma_dev():
    """Show a fun Karma Dev animation - terminal friendly with background colors"""
    karma_dev = [
        "  __  __                         ____                 ",
...
def setup_files():
    ...
    if not IDEAS_TXT.exists():
        ...
        with open(IDEAS_TXT, 'w') as f:
            f.write("# Karma Dev's Trading Strategy Ideas 🕉️\n")
...
def log_idea(idea, model_config):
    ...
    for phase in MOON_PHASES:
        clear_line()
        print(f"\r{colored(' ' + phase + ' Saving to Karma Dev database... ', 'white', 'on_magenta')}", end="", flush=True)
        time.sleep(0.3)  # Slower animation
    print()
...
def run_idea_generation_loop(interval=10):
    ...
    try:
        ...
        cprint("\n🕉️ Thank you for using Karma Dev's Research Agent! 🕉️", "white", "on_magenta")
    except Exception as e:
