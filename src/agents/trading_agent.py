...
class TradingAgent:
    ...
    def execute_position_closes(self, close_decisions):
        ...
        Delegates to the extracted _execute_position_closes function
        from the trade_executor module for cleaner code structure.

        Args:
            close_decisions: Dict mapping symbol to decision dict with 'action' and 'reasoning'
        """
        _execute_position_closes(
            close_decisions=close_decisions,
            close_position_fn=n.close_complete_position,
            get_position_fn=n.get_position,
            account=self.account,
            remove_from_tracker_fn=remove_position if POSITION_TRACKER_AVAILABLE else None,
            tracker_available=POSITION_TRACKER_AVAILABLE
        )
