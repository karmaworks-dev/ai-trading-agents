"""
🕉️ Karma Dev's OpenAI Model Implementation
...
class OpenAIModel(BaseModel):
    ...
    def initialize_client(self, **kwargs) -> None:
        ...
        try:
            ...
            cprint(f"✨ Karma Dev's magic initialized OpenAI model: {self.model_name} 🌟", "green")
    ...
    def _prepare_model_kwargs(self, **kwargs):
        ...
        if self._supports_reasoning_effort():
            cprint("🚀 Karma Dev's O3 model powering up with reasoning capabilities! 🕉️", "cyan")
    ...
    def generate_response(self, system_prompt, user_content, **kwargs):
        ...
        try:
            ...
            if self.model_name.startswith('o3'):
                cprint("🧠 Using Karma Dev's O3 model with reasoning capabilities...", "cyan")
                messages = [
                    {
                        "role": "user",
            ...
            try:
                ...
                cprint(f"🧪 Karma Dev debug: finish_reason={finish_reason}", "cyan")
            except Exception:
                pass
            ...
            if not content_text:
                cprint("🔁 Retrying once with simplified prompt format (Karma Dev fallback)", "yellow")
                retry_messages = [
                    {
                        "role": "user",
            ...
            if not content_text and hasattr(self.client, 'responses'):
                try:
                    cprint("🛠️ Karma Dev fallback: trying Responses API for text output", "yellow")
                    content_str = f"Instructions: {system_prompt}\n\nInput: {user_content}"
                    ...
                    cprint(f"❌ Karma Dev Responses API fallback error: {repr(fallback_e)}", "red")

            # Final safety net: fallback to a stable chat model if nothing came back
            if (not content_text) and self.model_name == 'gpt-5':
                try:
                    cprint("🛟 Karma Dev fallback: retrying with gpt-4o to avoid empty content", "yellow")
                    fallback_messages = [
            ...
            # Print detailed error info per Karma Dev style
            cprint(f"❌ OpenAI generation error (Karma Dev full dump) 🚨: {repr(e)}", "red")
            try:
