...
class WebSocketDataManager:
    ...
    def get_latest_candle(self, symbol: str, timeframe: str = None, days_back: int = None, bars: int = None):
        """
        Get latest OHLCV candle data for a symbol

        NOTE: This always uses API because WebSocket only provides real-time price updates.
        Historical OHLCV data requires batch fetching from the API.

        Args:
            symbol: Trading pair symbol (e.g., 'BTC', 'ETH')
            timeframe: Candle timeframe (e.g., '1m', '5m', '15m', '1h', '4h', '1d')
            days_back: Days of historical data (alternative to bars)
            bars: Number of bars to fetch (alternative to days_back)

        Returns:
            pd.DataFrame with OHLCV data and technical indicators, or None if unavailable
        """
        try:
            # Use config defaults if not provided
            if timeframe is None:
                try:
                    from src.config import DATA_TIMEFRAME
                    timeframe = DATA_TIMEFRAME
                except ImportError:
                    timeframe = '15m'

            # Calculate bars from days_back if provided
            if days_back is not None and bars is None:
                bars_per_day = {
                    '1m': 1440, '3m': 480, '5m': 288, '15m': 96, '30m': 48,
                    '1H': 24, '2H': 12, '4H': 6, '1h': 24, '2h': 12, '4h': 6,
                    '1D': 1, '1d': 1
                }
                bars = int(days_back * bars_per_day.get(timeframe, 24))
            elif bars is None:
                # Use default from config
                try:
                    from src.config import DAYSBACK_4_DATA
                    bars_per_day = {
                        '1m': 1440, '3m': 480, '5m': 288, '15m': 96, '30m': 48,
                        '1H': 24, '2H': 12, '4H': 6, '1h': 24, '2h': 12, '4h': 6,
                        '1D': 1, '1d': 1
                    }
                    bars = int(DAYSBACK_4_DATA * bars_per_day.get(timeframe, 24))
                except ImportError:
                    bars = 100

            return self.get_ohlcv_data(symbol, timeframe=timeframe, bars=bars, add_indicators=True)
        except Exception as e:
            logger.error(f"Failed to get candle data for {symbol}: {e}")
            return None
